...
contract BinaryConversionA {
    function BinaryToDecimal(string memory binary) public pure returns (uint) {
        ...
        while (i < bytes(binary).length) {
            ...
            // Convert character to integer value - subtracting ascii so 49-48 = 1 and 48-48 = 0
            uint8 digit = uint8(bytes(binary)[i]) - uint8(bytes1('0'));

            
            // Multiply prev result 2 and add the new digit - this way the 2 from i=0 1000 would get multiplied 3 times
    ...
    function BitShiftMask(uint8 inp ) public pure returns (uint8[] memory){
       // @dev implications of limiting to uint8 - limits input value to 255, 
       // yay for memory, yay for limiting the mask to max of 8 shifts
       uint8[] memory result = new uint8[](8); // storing result
       uint8 mask = 1;
       for (uint8 i = 0; i < 8; i++) {
            if ((inp & mask)!=0){
                result[i] = mask;
